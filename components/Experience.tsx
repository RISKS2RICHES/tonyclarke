...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="pt-10 pb-0 md:py-12 bg-navy-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 border-b-2 border-navy-200 pb-3">
          <h2 className="text-navy-900 text-2xl sm:text-3xl font-black uppercase tracking-tight">Professional History</h2>
        </div>

        <div className="space-y-4">
          {timeline.map((item, index) => (
            <div key={index} className="flex flex-col md:flex-row gap-3 md:gap-4 p-4 md:p-5 bg-white border border-navy-100 hover:border-navy-300 transition-colors">
              ...
              <div className="md:w-4/5">
                <h4 className="text-base sm:text-lg font-bold text-navy-900 mb-1">{item.title}</h4>
                <p className="text-xs sm:text-sm text-navy-700 font-semibold leading-relaxed">
                  {item.description}
                </p>
              </div>
            </div>
